...
fn transition_redraw_region(previous: Option<Rect>, current: Rect) -> Rect {
...
mod tests {
    ...
    fn transition_sequence_game_menu_high_scores_menu_uses_union() {
        ...
        let main_region = Rect {
            start_x: 40,
            end_x: 90,
            start_y: 6,
            end_y: 24,
        };
        let high_scores_region = Rect {
            start_x: 20,
            end_x: 110,
            start_y: 5,
            end_y: 25,
        ...
        let first = claim_redraw_region(main_region);
        assert_eq!(first, main_region);

        let second = claim_redraw_region(high_scores_region);
        assert_eq!(second, rect_union(main_region, high_scores_region));

        let third = claim_redraw_region(main_region);
        assert_eq!(third, rect_union(high_scores_region, main_region));
    ...
    fn renderer_sequence_menu_high_scores_menu_tracks_regions() {
        ...
        let menu_region = cached_region().expect("menu should populate a redraw region");
        ...
        let high_scores_region =
            cached_region().expect("high-scores should populate a redraw region");
        ...
        let menu_region_after_return =
            cached_region().expect("menu should restore its own cached redraw region");
